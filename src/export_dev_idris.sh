...
for section in train #test dev train
do
    for variant in va vb vc #v0
    do
...
UD_Spanish-AnCora/es_ancora UD_Turkish/tr
        ...
        array=(${treebank//\// })
        lang=${array[1]}
        python ud2mweconll.py --mwelists $variant.txt --output devsections/"$lang"_"$section"_"$variant".conll --trainfile $basepath/$treebank-ud-train.conllu --input $basepath/$treebank-ud-$section.conllu
        done
    done
done